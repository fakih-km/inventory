...
if(isset($_POST['btnSimpan'])){
	...
	//mengisi tabel po
		...
		$upSql ="UPDATE po SET nm_supplier='$cmbSupplier', divisi_tujuan='$txtDivisiTujuan' WHERE no_po='$noTransaksi'";
		$upQry = mysqli_query($koneksidb,$upSql);
		if ($upQry){
					$slSql ="SELECT kd_barang,jumlah,harga_beli FROM tmp_pengadaan WHERE no_po='$noTransaksi'";
					...
					// $upQry = mysqli_query($koneksidb,$upSql);
						...
						while($myDatax = mysqli_fetch_array($slQry)){
							...
							$sl1Sql="SELECT po_item WHERE no_po='$noTransaksi' AND kd_barang='$myDatax[kd_barang]";
							...
							$tmpInsert = "INSERT INTO po_item (no_po, kd_barang, nm_barang, jumlah, harga)
								VALUES ('$noTransaksi', '$myDatax[kd_barang]',
...
$mySql ="SELECT a.no_po, a.tgl_po, a.divisi_tujuan, b.nm_suplier,b.kd_suplier, a.tanggal, a.total FROM po a LEFT JOIN suplier b ON a.nm_supplier=b.kd_suplier
	 WHERE a.no_po= '$kode'";
...
$dataSupplier = isset($_POST['cmbSupplier']) ? $_POST['cmbSupplier'] : $myData['kd_suplier'];
...
$tSql = "INSERT INTO tmp_pengadaan(kd_barang,no_po,jumlah,harga_beli,kd_petugas) 
SELECT kd_barang,no_po,jumlah,harga,'$userLogin' FROM po_item WHERE no_po='$noTransaksi'";
...
<table id="datatable" class="table table-striped table-bordered">
	...
	$tmpSql ="SELECT tmp_pengadaan.*, barang.nm_barang FROM tmp_pengadaan, barang WHERE tmp_pengadaan.kd_barang=barang.kd_barang AND tmp_pengadaan.kd_petugas='$userLogin' AND tmp_pengadaan.no_po='$noTransaksi' ORDER BY id";
	$tmpQry = mysqli_query($koneksidb, $tmpSql);
	...
	<tr>
		...
		<td align="center">
			<?php if (!$_SESSION['SES_PENGGUNA']):?>
				<input type="button" onclick="modalEdit('<?php echo $tmpData[kd_barang]?>','<?php echo $tmpData[nm_barang]?>','<?php echo $tmpData[harga_beli]?>','<?php echo $tmpData[jumlah];?>')" data-toggle="modal" data-target=".modal-tambah<?php echo $noTransaksi; ?>"></input>
