...
$Kat 			= isset($_GET['Sup']) ? $_GET['Sup'] : 'semua';//dari URL
$dataSuplier 	= isset($_POST['cmbSuplier']) ? $_POST['cmbSuplier'] : $Kat; //dari form
if (trim($dataSuplier)=="semua") {
		$filterSQL ="";		
	}
	else {
		$filterSQL="WHERE suplier.kd_Suplier='$dataSuplier' ";
}
?>
	...
	<div class="form-group">
		...
		<div class="col-md-6 col-sm-6 col-xs-12">
			<select id="aaa" class="form-control" name="cmbKategori">
				...
				$dataSql ="SELECT * FORM suplier ORDER BY kd_Suplier";
				$dataQry = mysqli_query($koneksidb, $dataSql);
				while ($dataRow = mysqli_fetch_array($dataQry)) {
					if ($dataRow['kd_Suplier']== $dataSuplier){
						$cek ="selected";

					} else {$cek="";}
					echo "<option value='$dataRow[kd_Suplier]' $cek>$dataRow[nm_Suplier]</option>";
				}
...
<table id="datatable" class="table table-striped table-bordered">
	<thead>
		<tr>
			...
			<th width="200"><strong>Nama Suplier</strong></th>
			<th width="300">Alamat</th>
			<th width="132">Telepon</th>
			<th width="70" align="center"><strong>Jumlah</strong></th>
			<th width="100" align="center"><a href="<?php echo $baseURL;?>suplier/addsup" target"_self">
				<span class="fa fa-plus-circle"></span> Add Data</a></th>
		</tr>
	</thead>
	<?php
	/* PENCARIAN BERDASARKA DATA DI TABEL*/
	$mySql ="SELECT * FROM suplier $filterSQL ORDER BY nm_Suplier DESC, kd_Suplier DESC";
	$myQry = mysqli_query($koneksidb, $mySql);
	$nomor = $hal;
	//PERULANGAN DATA
	while ($myData = mysqli_fetch_array($myQry)) {
		$nomor++;
		$Kode = $myData ['kd_Suplier'];
		?>
		
	<!--MENAMPILKAN HASIL PENCARIAN DATABASE-->
	<tr>
		<td align="center"><?php echo $nomor;?></td>
		<td><?php echo $myData['kd_Suplier']?></td>
		<td><?php echo $myData['nm_Suplier']?></td>
		...
		<td align="center">
			<?php if (!$_SESSION['SES_PENGGUNA']):?>
				<a href="<?php echo $baseURL;?>suplier/editsup?kode=<?php echo $Kode; ?>">
					<span class="fa fa-pencil"></span></a>
				<a href="<?php echo $baseURL;?>suplier/deletesup?kode=<?php echo $Kode; ?>" target="_self" alt="Delete Data" onclick="return confirm('Apakah anda yakin ingin menghapus data <?php echo $Kode; ?>')">
